...
@create_category_router.message(or_f(F.text.lower() == "/добавить категорию", F.text.lower() == "добавить категорию"), or_f(CheckAdmin(), CheckSuperAdmin("6102015555")))
async def create_category(message: Message, state: FSMContext, bot: Bot):
    db = DataBase()
    lang = await db.get_lang(str(message.from_user.id))
    await bot.send_message(str(message.from_user.id), "Пожалуйста, введите название категории, которую вы хотите создать: ",
