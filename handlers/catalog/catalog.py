...
async def decrease_quantity(call: CallbackQuery, state: FSMContext):
    ...
    current_quantity = max(1, state_data.get(f"quantity_{product_id}", 1) - 1)
...
async def confirm_order(call: CallbackQuery):
    ...
    try:
        ...
        if product.quantity - quantity == 0 and call.message.reply_markup:
            await call.message.edit_reply_markup(reply_markup=None)
...
async def add_basket(call: CallbackQuery):
    ...
    if product.quantity - quantity == 0:
        # Remove the reply markup if no stock is left
...
async def sold_out(call: CallbackQuery):
    db = DataBase()
    lang = await db.get_lang(str(call.from_user.id))
    await call.message.answer(_(""""Мы сожалеем! 🛒"

Интересующий вас продукт в данный момент распродан. 😔""", lang))
    await call.answer()
